...
func CreateVice(tx *pop.Connection) error {
	a := models.Author{Name: "Mat Ryer"}
	err := tx.Create(&a)
	if err != nil {
		return errors.WithStack(err)
	}

	tm, err := time.Parse(time.RubyDate, "Mon Aug 17 17:27:37 -0000 2017")
	if err != nil {
		return errors.WithStack(err)
	}
	v := models.Version{Tag: "v0.1.0", ReleasedOn: tm}
...
const viceDescription = `Go channels at horizontal scale (powered by message queues)`

const viceLicense = `                                 Apache License
